#!/bin/bash
if [[ !($1 =~ ^(RR|ML|FIX|MIN|QCK|CAS|ALL)$)  ||
      !($2 =~ ^[0-9]+$)           ||
      !($3 =~ ^(static|dynamic)$) ||
      !($4 =~ ^(steal|no-steal)$) ]]; then
    echo "usage: ./run.sh [RR|ML|FIX|MIN|QCK|CAS|ALL] [repeat_count] [static|dynamic] [steal|no-steal]"
    exit 1
fi
...
for (( i = 0; i < $2; i++ )); do
    if [ $1 == ALL ]; then
        ...
        python3 main.py --allocator CAS $options
